...
public class ChunkClaim {
  ...
  public ChunkClaim(Chunk claim, UUID owner, List<UUID> trusted) {
    this.id = (SurvivalPlugin.ccm.lastId + 1);
    SurvivalPlugin.ccm.lastId = (SurvivalPlugin.ccm.lastId + 1);
  ...
  public void saveToDatabase() {
    try(Connection connection = SurvivalPlugin.dataSource.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement("insert into ChunkClaims values ( ?, ?, ?, ?, ?, ?)")) {
  ...
  public void updateDatabase() {
    try(Connection connection = SurvivalPlugin.dataSource.getConnection(); PreparedStatement preparedStatement = connection.prepareStatement("update ChunkClaims set trusted = ? where id = ?")) {
      StringJoiner joiner = new StringJoiner(",");
      for (UUID uuid : this.trusted)
        joiner.add(String.valueOf(uuid)); 
